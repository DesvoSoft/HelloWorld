document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!wordsContainer || !timerDisplay || !newGameBtn || !durationSelect || !modeSelect || !resultDisplay) return;
  ...
  // Frases comicas que devs y usuarios de IA repiten sin parar
  const promptPhrases = [
    "just make it work",
    "please dont break anything",
    "ignore all previous instructions",
    "make it pop",
    "ship it we are already late",
    ...
    "one more console log wont hurt",
    "chatgpt wrote this dont judge me",
    "we will refactor later",
    "just use a global variable",
    "copy paste from stack overflow",
    "works in dev breaks in prod",
    "add a comment nobody will read",
    "commit message fix stuff",
    "force push and pray",
    "rm rf and hope for the best",
    "blame the intern",
    "it is not a bug it is a feature",
    "cache is the problem it is always the cache",
    "as an ai language model i cannot",
    "can you fix this real quick",
    "trust me it is fine",
    "works for me shrug",
    "act as a senior developer",
    "make it more professional",
    "can you make it better",
    "why does this only break in production",
    "who wrote this code",
    "git blame says it was me"
  ];

  // Genera texto aleatorio combinando frases completas del modo prompts
  ...
  function loadWords() {
    ...
    if (modeSelect.value === "prompts") {
      words = generatePromptText();
    } else if (modeSelect.value === "terry") {
  ...
  function endGame() {
    ...
    const resultMessage = `
      <div class="resultBox">
        <h2>Resultados</h2>
        <p><strong>Palabras:</strong> ${totalWordsTyped}</p>
        <p><strong>WPM:</strong> ${wpm}</p><br>
        ${errors ? `<p><strong>Errores:</strong><br>${errors}</p>` : ""}
      </div>
    `;
